...
class MaskedTokenPredictorBert:
    def __init__(
        ...
        self.mean = mean  # could be scipy.stats.hmean, but for negative tokens
        self.confuse_bert_args = confuse_bert_args   # reproduce an old error for backward compatibility
    ...
    def predict_single_unit(self, bpe_tokens, masked_position, 
        ...
        for i in range(target_sent.shape[0]):
            ...
            if idx + 1 >= len(row):
                # todo: investigate why this shit happens when n_units>1
                continue
            logits = row[idx + 1]
